...
int Solver::idaSearch(Board& board, int g, int bound, MoveDirection previous) {
    ...
    for (MoveDirection dir : moves) {
        ...
        // 尝试当前分支
        board.move(dir);
        idaPath.push(dir); // 【修改点】：将 push_back 改为 push
        ...
        // 尝试其他分支前，恢复棋盘和路径
        idaPath.pop();     // 【修改点】：将 pop_back 改为 pop
...
SolveResult Solver::solveByIdaStar(const Board& start) {
    ...
    expanded = 0;
    idaPath.clear();                 // SeqStack 支持 clear()

    while (bound <= idaLimit) {
        int value = idaSearch(board, 0, bound, MOVE_NONE); // 返回 -1 表示找到目标。
        if (value == -1) {
            // 【修改点】：利用辅助栈，将路径由栈导出到 vector 并反转
            std::vector<MoveDirection> path;
            SeqStack<MoveDirection> tempPath = idaPath; // 深拷贝当前路径栈
            while (!tempPath.empty()) {
                path.push_back(tempPath.pop()); // 依次弹出末尾步骤
            }
            std::reverse(path.begin(), path.end()); // 翻转得到正常的正向移动路径
