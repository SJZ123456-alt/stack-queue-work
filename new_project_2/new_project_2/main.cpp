...
int currentN = 3;
Board currentBoard;
Board initialBoard; // 记录开局状态，现在正好派上用场！
Solver solver;
...
void processVictory() {
    ...
    if (res.solved && humanSteps == res.steps) {
        _stprintf_s(msg, _T("太强了！你用了 %d 步完成！\n这是理论最少步数，你就是华容道之神！"), humanSteps);
    }
    else {
        _stprintf_s(msg, _T("恭喜通关！你用了 %d 步。\n（提示：理论最少步数为 %d 步）"), humanSteps, res.solved ? res.steps : 0);
    }
...
void handleMouseClick(int mx, int my) {
    ...
    if (mx >= BTN_X && mx <= BTN_X + BTN_WIDTH) {
        ...
        // 2. 重新开始本局 (恢复到本局刚开始的样子，步数清零)
        if (my >= 140 && my <= 140 + BTN_HEIGHT) {
            currentBoard = initialBoard; // 恢复初始状态
            humanSteps = 0;              // 步数清零
            state = PLAYING;             // 状态设为正在游玩
        }
...
void checkSolvable() {
    if (currentBoard.isSolvable()) {
        MessageBox(GetHWnd(), _T("放心玩，这个棋盘是有解的！"), _T("检测结果"), MB_OK);
    }
    else {
        MessageBox(GetHWnd(), _T("数学上证明此局无解，别折磨自己了，请点击【新的一局】！"), _T("检测结果"), MB_OK | MB_ICONWARNING);
    }
}

void showSolution() {
    if (!currentBoard.isSolvable()) {
        MessageBox(GetHWnd(), _T("当前棋盘无解，无法演示！"), _T("提示"), MB_OK);
        return;
    }
    ...
    if (res.solved) {
        TCHAR msg[128];
        _stprintf_s(msg, _T("计算完毕！最少还需要 %d 步。\n点击确认开始演示。"), res.steps);
        MessageBox(GetHWnd(), msg, _T("AI 准备就绪"), MB_OK);
        ...
        state = GAME_OVER;
        MessageBox(GetHWnd(), _T("演示结束，请点击【新的一局】"), _T("提示"), MB_OK);
    }
}
...
void drawUI() {
    ...
    if (state == GAME_OVER) {
        ...
        outtextxy(BTN_X, 360, _T("请点击【新的一局】"));
    }
}